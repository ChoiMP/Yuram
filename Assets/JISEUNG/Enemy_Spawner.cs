    ...
    void Update()
    {
        if (boss_C == false)
        {
            if (boss_Time <= 0)
            {
                ...
                boss_C = true;
                jihoCanvas.Instance.ChangeMusic(jihoCanvas.Instance.soundClip[1]);
            }
        ...
        if (spawn_Time <= 0)
        {
            ...
            for (int i = 0; i <= spawn_Count_; i++)
            {
                ...
                {
                    GameObject clone = Instantiate(spawn_Enemy[spawn_Ememy_Num], spanwer_Point[spawn_Point_Num].transform.position, Quaternion.identity);
